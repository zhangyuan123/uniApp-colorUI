<template>
	<view class="content">
		...
		<scroll-view scroll-x class="bg-white nav text-center fixed" :style="{top:CustomBar + 'px'}">
			<view class="cu-item" :class="index == TabCur ? 'text-blue cur' : ''" v-for="(item, index) in tabNav" v-key='index'
			 @click="tabSelect" :id='index'>{{tabNav[index]}}</view>
		</scroll-view>
		<block v-if="TabCur == 0">
			<view class="cu-bar bg-white solid-bottom margin-top" style="margin-top: 120rpx;">
				<view class="action">
			...
			<view class="padding bg-white">
				<view class="flex flex-wrap">
					<view class="basics-xs bg-grey margin-xs padding-sm radius">xs(20%)</view>
		...
		<block v-if="TabCur == 1">
			...
			<view class="bg-white padding">
				<view class="grid margin-bottom text-center" :class="'col-' + (index + 1)" v-for="(item, index) in 5" v-key='index'>
					<view class="padding" :class="childId%2 == 0 ? 'bg-cyan' : 'bg-blue'" v-for="(child, childId) in (index + 1)*2">{{childId + 1}}</view>
				</view>
			</view>
			<view class="cu-bar bg-white solid-bottom margin-top">
				<view class="action">
			...
			<view class="padding bg-white">
				<view class="grid col-4 grid-square">
					<view class="bg-img" v-for="(item, index) in 4" v-key='index'>
						<image :src="bgImgStr + (index+1) + '.jpg'" mode="aspectFill"></image>
					</view>
				</view>
			</view>
		</block>
		<block v-if="TabCur == 2">
			...
			<view class="padding bg-white">
				<view class="cf padding-sm">
					<view class="bg-grey fl radius padding-sm">Colorui fl</view>
					<view class="bg-grey fr radius padding-sm">Colorui fr</view>
				</view>
			</view>
			<view class="cu-bar bg-white solid-bottom margin-top">
				<view class="action">
